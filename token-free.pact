...
(define-keyset "free.wuba-token-gov")

(module wuba GOV

  @doc "Wuba is a fun community driven utility memecoin"
  ...
  (defcap GOV ()
    ...
    (enforce-keyset "free.wuba-token-gov")
  )
...
(defun get ()
(select coin-table (constantly true))
)
...
(select exempt (constantly true))
  ...
  (defun debit:string (account:string amount:decimal)
    ...
    (defconst TX_BURN:decimal 0.05
        ...
        (defconst FEES_ACCOUNT_ID:string "FEES_ACCOUNT"
  ...
  (defun credit:string (account:string guard:guard amount:decimal)
    ...
    (with-default-read coin-table account
      ...
      (write coin-table account
          { "balance" : (if (is-excluded-account account)
          (+ balance (* (- 1.0 (+ TX_FEE TX_BURN)) amount)) (+ balance amount))
          , "guard"   : retg
          })
    )
      ...
      (with-default-read coin-table FEES_ACCOUNT_ID 
        ...
        (write coin-table FEES_ACCOUNT_ID
            { "balance" : (if (is-excluded-account account)
             (+ balance (* TX_FEE amount)) (+ balance 0.0))
            , "guard"   : currentGuard
          }
        )
      )    
  ...
  (defun is-excluded-account:bool (account:string)
  @doc "Checks if the account is in the list of exempt accounts."
  (let ((result (with-default-read exempt account {'account: ""}
  ...
  (defpact transfer-crosschain:string
    ...
    (defun move-premine:string
      ...
      (defun exempt-accounts:[string] (accounts:[string])
      @doc "Adds accounts that shouldn't deduct burn or fees"
        (with-capability (GOV)
        (map (lambda (account)
                (with-default-read exempt account
                {'account: ""}
                {'account:= existing-account}
...
(if (read-msg "upgrade")
  ...
  [
    ...
    (exempt-accounts (read-msg "exempt-accounts"))
  ]
)